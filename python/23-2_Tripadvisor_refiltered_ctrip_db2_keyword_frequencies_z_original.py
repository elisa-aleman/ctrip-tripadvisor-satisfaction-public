...
def keyword_frequencies__ctrip_db2_filtered_date_hotel_name():
    tagged_df = get_Tripadvisor_tagged_english_sentences__ctrip_db2_filtered_date_hotel_name_df()
    ...
    ins_path = make_data_path("tripadvisor/tripadvisor_db/keyword_frequencies_Combined_Keywords_z_original_p1.5_n4.25_english_only_filtered_ctrip_db2_date_hotel-name.csv")
    ins_df.sort_values(by=["Positive Absolute Frequency"], ascending=False,inplace=True)
    ins_df.to_csv(ins_path, index=False)

def keyword_frequencies__ctrip_db2_filtered_date_hotel_price():
    tagged_df = get_Tripadvisor_tagged_english_sentences__ctrip_db2_filtered_date_hotel_price_df()
    keyword_list = get_Tripadvisor_original_selected_keywords()
    pos_list = get_Tripadvisor_original_selected_positive_keywords()
    # neg_list = get_Tripadvisor_original_selected_negative_keywords()
    pos_count_dict = dict.fromkeys(keyword_list,0)
    ...
    for keyword in keyword_list:
        ...
        ins_table.append(ins_row)
    ins_df = pandas.DataFrame(ins_table, columns=["Keyword","Emotion List","Positive Absolute Frequency","Positive Relative Frequency","Positive Probability per sentence","Negative Absolute Frequency","Negative Relative Frequency", "Negative Probability per sentence"])
    ins_path = make_data_path("tripadvisor/tripadvisor_db/keyword_frequencies_Combined_Keywords_z_original_p1.5_n4.25_english_only_filtered_ctrip_db2_date_hotel-price.csv")
    ins_df.sort_values(by=["Positive Absolute Frequency"], ascending=False,inplace=True)
    ins_df.to_csv(ins_path, index=False)

if __name__ == '__main__':
    keyword_frequencies__ctrip_db2_filtered_date_hotel_name()
    keyword_frequencies__ctrip_db2_filtered_date_hotel_price()
