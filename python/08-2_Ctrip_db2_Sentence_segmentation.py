...
def split_sentences():
    ...
    for ind,row in enumerate(comments_df.itertuples()):
        ...
        for sent_num,sentence in enumerate(sentences):
            ...
            ins_row = (row.HotelID,row.CommentPage,row.CommentID,sent_num+1,sent_text)
            ins_table.append(ins_row)
        up()
    down()
    print("Insert segmented sentences")
    ins_table_df = pandas.DataFrame(ins_table,columns=["HotelID","CommentPage","CommentID","SentenceNumber","Sentence"])
    ins_table_df.index += 1
    ins_table_df.index.name = 'RID'
    ins_path = make_data_path("ctrip/ctrip_db2/sentences_zh-cn_chinese_only.csv")
