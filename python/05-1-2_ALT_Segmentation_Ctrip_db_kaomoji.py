...
def PreliminarySegmentation():
    ...
    for ind,row in enumerate(comments_df.itertuples()):
        ...
        # HotelID,CommentID,Segmented_Comment
        ins_kao_row = (row.HotelID,row.CommentID,seg_text_tagged)
        ins_kao.append(ins_kao_row)
        up()
    down()
    print("Insert segmented with tags for kaomoji")
    ins_kao_df = pandas.DataFrame(ins_kao,columns=['HotelID','CommentID','Segmented_Comment'])
    ins_kao_df.index += 1
    ins_kao_df.index.name = 'RID'
    ins_path = make_data_path("ctrip/ctrip_db/comments_segmented_kaotag_zh-cn_chinese_only.csv")
    ...
    for item in ins_kao:
        ...
        ins_row = (item[0],item[1],segmented)
        ins.append(ins_row)
    ins_df = pandas.DataFrame(ins,columns=['HotelID','CommentID','Segmented_Comment'])
    ins_df.index += 1
    ins_df.index.name = 'RID'
    ins_path = make_data_path("ctrip/ctrip_db/comments_segmented_zh-cn_chinese_only.csv")
